...
(define (remove-debtors act)
  (local [(define (f act) (negative? (node-bal act)))]
    (fold-acc f false act)))
...
(define (remove-profs act)
  (local [(define (f act) (has-prefix? "Prof." (node-name act)))]
    (fold-acc f false act)))
...
;; Accounts -> Accounts
;; Removes from a given BST, any account where the name of the account holder has an odd no. of characters
...
(define (remove-odd-characters act)
  (local [(define (f act) (odd-characters? (node-name act)))
          (define (odd-characters? name)
            (odd? (string-length name)))]
    (fold-acc f false act)))
...
;; Accounts -> Accounts
;; Decreases the balance of every account in a given collection by the monthly fee of 3 CAD
(check-expect (charge-fee ACT1) (make-node 1 "Mr. Rogers"  19 false false))

(define (charge-fee act)
  (local [(define (f id name bal lact ract)
            (make-node id name (- bal 3) lact ract))]
...
;; Use fn-for-act as fold-act searches the whole tree even if it has already found the account
