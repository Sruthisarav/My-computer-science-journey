...
(time (ouster "Nemesis" false))
(time (ouster "Scandal" BN))
(time (ouster "Nemesis" BN))
(time (ouster "Traffic" BN))

;; Local expression is used to reduce the time taken to process the function as
;; (ouster t (bracket-br-won br)) is run twice in the initial function
