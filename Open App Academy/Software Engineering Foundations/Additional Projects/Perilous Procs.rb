...
p filter_out([1, 7, 3, 5 ]) { |x| x.odd? }         # [1, 7, 3, 5]
...
def multi_map(arr, n=1, &prc)
    newArr = arr
    n.times do 
        newArr = newArr.map {|ele| prc.call(ele)}
    end
    return newArr
end
...
def chain_map(value, array)
    result = value
    array.each {|proc| result = proc.call(result)}
    return result
end
...
def proc_suffix(sentence, hash)
    ...
    arr.each_with_index do |word, i|
        hash.each do |proc, suffix|
            arr[i] += suffix if proc.call(word)
        end
    end
    return arr.join(" ")
end
...
def proctition_platinum(arr, *procs)
    hash = {}
    (1..procs.length).each do |n|
        count = 0
        hash[n] = []
        return hash if arr.empty?
        (0...arr.length).each do |i|
            ele = arr[i-count]
            if procs[n-1].call(ele)
                hash[n] << ele 
                arr.delete(ele)
                count += 1
            end
...
def procipher(sentence, hash)
    ...
    words.each_with_index do |word, i|
        hash.each do |k, v|
            words[i] = v.call(word) if k.call(word)
        end
    end
    return words.join(' ')
end
